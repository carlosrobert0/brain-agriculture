...
import { Dialog, DialogContent, DialogTrigger, DialogClose, DialogTitle } from "@radix-ui/react-dialog"
...
export default function CropsPage() {
  ...
  const { onDeleteCrop } = useCrop()
  const { getHarvestInfo } = useHarvest()
  const { crops, harvests } = appState
            ...
            <Card key={crop.id}>
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    <DialogTitle className="text-lg">{crop.name}</DialogTitle>
